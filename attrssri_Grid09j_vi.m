function optPar = attrssri_Grid09j_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest)
%  optPar = attrssribGrid09j_vi( block, wave) :  MAP fitting, with weakly informative priors given by 
%                        scaled beta distributions. ssria means the first wave, and 09b means
%                        that it's the 09 version of the generative function, j for a large set
...
%% Cater for any Fixed value parameters ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
% parNOTTOGRID = [12, 3];  % NO parameters-not-to-grid-over in 09j
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
cwd = pwd;
wrkDir = ['C:\Users\mmpsy\Dropbox\BASOR\AcInSOR\AcInRepeatedDictator\attrssri' wave '\'];
try
   cd(wrkDir);
catch
    try
      wrkDir = ['/home/mmoutou/Dropbox/BASOR/AcInSOR/AcInRepeatedDictator/attrssri' wave '/'];
      cd(wrkDir);
    catch
       wrkDir = ['/home/michael/Dropbox/BASOR/AcInSOR/AcInRepeatedDictator/attrssri' wave '/'];
       cd(wrkDir);       
    end
end
...
for ptN = todo
    ptCnt = ptCnt+1; 
    parOrdGrid = [1 2 8 3 5 11 12 4 6 7 9 10 1 2 8 3 5 12 7 6 4 9 1 2 11 12];  % in order above in hd. Cycle twice!
    if codeTest; parOrdGrid = [8 12 2 12]; end
    parCnt = 0;
...
%                 end
        ...
        save([fitFileName '.mat'],'expGrid','optPar','hd','parName');
    end
