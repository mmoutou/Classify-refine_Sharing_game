function optPar = attrssri_Grid09q_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest)
%  attrssriGrid09q_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest) :  
%                        MAP fitting, with weakly informative priors given by 
%                        scaled beta distributions, by estimating sum LL over a grid.
%               09q means that it's the 09 version of the generative function, q for a set
%                        of parameters to be fitted/considered described in code section 
%                        'Cater for Fixed value parameters' below, e.g. EXCLUDING POC BIAS, and
%                        setting (transf) initEvRatTrRef to 0. 
%                        Fitting being done with simplest learning from one block to the other,
%                        adding admixture of the posterior to the prior.
%               vi is the version of the likelihood fn used, 
%               block    is an argument about which set of blocksize (penultimate argument)
%                        participants to use.
%               blocksize is how many pts to fit for one run of this function, i.e. serially. 
%                        Fitting all of them serially is impractical.
%               startFitFile contains the initial values of the parameters
% To add / remove params to grid over, change: 0. Word search 'Fixed value' and set *TrRef values
% 1. parNOTTOGRID 2. par2fitN  3. parOrdGrid 4. fitFileName 5. Check BIC and AIC estimates
% NB hd and totParN NOT to be changed for this purpose.
% 
% Example use, here for 2nd block of 16 pts at wave b, grN=12, grSc=5, no code testing:
...
%     It is then recommended that different nested-parameter-set versions are compared, which will
%     sometimes show that more complext models got did not find the max. LL that simpler models found,
%     and need to be re-initialized with that best LL.
...
parNOTTOGRID = [12, 5]; 
pocBTrRef = log(1.0); 
initEvRatTrRef = log(1.0);      % REM from tr2nat... :  natP.initEvRat = exp(P.initEvRat);   
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
cwd = pwd;
wrkDri = pwd; % Change this to your favourite, e.g. a separate one for each wave.
try
   cd(wrkDri);
catch
    try
...
% will be used as a template or scaffold for all the Fixed value params.
...
totParN=indexPLen+2; % total number of params in indexP, plus othLR and pocB.
totOthN=4; 
par2fitN=totParN-length(parNOTTOGRID);  % We would like to have the choice to fit any of the 
           % totParN params (in modStuc.indexP plus othLR and pocB) but not nessarily all of them. 
...
for ptN = todo
    ptCnt = ptCnt+1; 
    % In 09q, no pocB (12) nor initEvRat (5) : 
    parOrdGrid = [1 2 9 8 3 11 4 6 7 9 10 1 2 8 3 7 6 4 9 10 1 2 11];  % in order above in hd. Cycle twice!
    if codeTest; parOrdGrid = [8 11 2 11]; end    
    % was: parOrdGrid = [1 2 8 3 5 11 12 4 6 7 9 10 1 2 8 3 5 12 7 6 4 9 10 1 2 11 12];  % in order above in hd. Cycle twice!
    parCnt = 0;
    for par2grid = parOrdGrid
        ...
        if parCnt == 1
           ...
           if isempty(ldat)    % if no good init. cond. loaded
                ...
                % Params 11 to 12, not used inside MDPs but between them:
                ...
                pocBTr = pocBTrRef;              % Fixed value of pocBTr on real line.
                pocB = exp(pocBTr); 
                ...
                mapMod.mdpStruc.indexP.initEvRat = initEvRatTrRef;   % fixed value 'removing the param'
...
%                 end
        ...
        for iGr = -grN:grN      
            ...
            if ~isempty(find(par2grid == parNOTTOGRID, 1))  % Shouldn't get to this!
                disp('  OUCH '); 
            ...
            if par2grid == totParN-1     % i.e. if stepping the others-lr
                refp = othLRTr;
                thisp =  refp + iGr * (abs(refp) + 1)/grSc;                 
            ...
            %%  ################ Fixed values ###############
            mapMod.mdpStruc.pocBTr = pocBTrRef;                 % fixed unbiased value on real line
            mapMod.mdpStruc.indexP.initEvRat = initEvRatTrRef;  % ditto ...
            ...
            for blockN = 1:length(expD{ptN}.other)
                ...
                mapMod.mdpStruc.indexP.pH0 = log( pH0^(1/coalB)  /( 1-pH0^(1/coalB)) ); 
        ...
        refMapMod.mdpStruc.pocBTr = mapMod.mdpStruc.pocBTr;  % pocB is constant in 09q.
        ...
        %  AIC_kc derived very slopily indeed, but c=0 here, as no group-level-derived pars in 09q:
        %         AIC_kc  := -2 ln Lopt + 2*((k-c)  + c/N)
        optPar(ptCnt,totParN+3) = -2*sumLL + 2*par2fitN ; % + ...
                                             % 2/totPtN ;     % roughly corrected Akaike IC (AIC)
        ...
        optPar(ptCnt,totParN+4) = -2*sumLL + par2fitN*(log(datNPerPt) - log(2*pi)) ;  % + ...
                                   % + (1/totPtN)* log( totPtN * datNPerPt) ;  % Bayesian IC
                                   % using formula BIC for small data sample per pt.      
