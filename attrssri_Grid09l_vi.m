function optPar = attrssri_Grid09l_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest)
...
par2fitN=indexPLen;   % We would like to have the choice to fit any of the params in modStuc.indexP
...
for ptN = todo
    ptCnt = ptCnt+1; 
    % in 09l, missing 12 (pocB) and 7 (mem) :
    parOrdGrid = [1 2 8 3 5 11 4 6 9 10 1 2 8 3 5 6 4 9 1 2 11];  % in order above in hd. Cycle twice!
    ...
    for par2grid = parOrdGrid
        ...
        if parCnt == 1
           ...
           if isempty(ldat)    % if no good init. cond. loaded
                ...
                natPinit.mem  =invlogit(memTrRef);   % memory / forgetting default
