function mL = spm_mdp_L_vi(P,modStruc,Inp,Resp,details)
% minus-log-likelihood function, developped from spm_mdp_L to go with serialDictator09b - roman num index _i, _lxix etc.
% FORMAT mL = spm_mdp_L_vi(P,modStruc,Inp,Resp)
...
for i = 1:Plen
    % first, log-transformed: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     elseif strcmp(field{i},'wH')
        ...
        if abs(Pall.wH - exp_wH) >  tiny
           Pall.wH = exp_wH;   aHubAdj=1;     pFHSAdj = 1;
        end
    elseif strcmp(field{i},'wS')
        exp_wS = exp(P.wS);
        if abs(Pall.wS - exp_wS) >  tiny
           Pall.wS = exp_wS;   aHubAdj=1;     pFHSAdj = 1;
        end    
    ...
    elseif strcmp(field{i},'w0')
        if abs(Pall.w0 - P.w0) >  tiny
         Pall.w0 = P.w0;   aHubAdj=1;       pFHSAdj = 1;
        end
...
%% pFHSAdj needed to update d in the attribution MDPs acc. to current mdpHub
%  i.e. below in [mdpH, mdpS] = hub2spoke_dAttr(mdpH, mdpS, hmmHub(tr), Pall)
if pFHSAdj 
    ...
    Pall.pFHiLoHS = pFairHiLoAttr([Pall.w0,Pall.wH,Pall.wS]);  % provides pFHS, attrV, iLo, iHi, iHS
end

%%
if   aHubAdj
    a0 =  AHubClassSerDict(Pall);  
    a0(1:2,:,:) = a0(1:2,:,:) * Pall.aInitEv; 
    hmmHub.a{1} = a0; 
    if isempty(Inp)     % i.e. we have not been given returns etc, so need to generate them
        hmmHub.A = {};
        hmmHub.A{1} = AHubClassSerDict(Pall, Pall.trueAttrV(1,:),Pall.trueAttrV(2,:));
    end
end
...
if  dHubAdj 
    d0 = dHubClassSerDict(Pall);   % Will need copy before it's updated!
    hmmHub.d     =  d0; 
end

% Adjust report MDP maps acc to parameters, if necessary  --
if  ARepAdj
    mdpHI.A{2} = ARepClassifSerDict(Pall);   
    mdpSI.A{2} = mdpHI.A{2};  
...
for tr = 1:trialN
  ...
  if tr > 1
     ...
     mdpF.D = hub2spoke_D_Fair(hmmHub(tr-1).a{1}, hmmHub(tr-1).d, Pall);
  else
     mdpF.D = hub2spoke_D_Fair(a0, d0, Pall);  % use copies from before hmmHub model ran at all!
  end
  ...
  [mdpH, mdpS] = hub2spoke_DAttr_vi(mdpH, mdpS, hmmHub(tr), Pall); 
