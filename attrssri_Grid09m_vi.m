function optPar = attrssri_Grid09m_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest )
%  optPar = attrssribGrid09m_vi( block, wave) :  MAP fitting, with weakly informative priors given by 
% attrssriaGrid09m_vi_**  MAP fitting, with weakly informative priors given by 
...
%                        of parameters to be fitted/considered EXCLUDING POC BIAS and mem and
%                        setting wS == wH. 
...
parNOTTOGRID = [12, 7];   % pocB and mem fixed, wS will be constrained but not fixed.
pocBTrRef = log(1.0);  
memTrRef =  6.9068; % 9.210240366975960;   % mem set to no-forgetting, logit(0.999)9)
...
for ptN = todo    % THIS COUNTS IN THE ORDER OF THE DATA OF EACH WAVE, WHICH 
                  % ARE DIFFERENT. 
    ptCnt = ptCnt+1; 
    % in 09m, missing 12 (pocB) and 7 (mem) and 9 (wS) :
    parOrdGrid = [1 2 8 3 5 11 4 6 10 1 2 8 3 5 6 4 1 2 11];  % in order above in hd. Cycle twice!
    ...
    for par2grid = parOrdGrid
        ...
        for iGr = -grN:grN      
            ...
            if par2grid <= totParN-2  % i.e. if it's not the learning-between-others learning rate
                ...
                if par2grid == 8   % Now if we are stepping wH, also step wS together:
                   mapMod.mdpStruc.indexP.(fields4MDPs{9}) = thisp;
                end   
        ...
        if par2grid <= totParN-2
           ...
           if par2grid == 8   % Now if we are stepping wH, also step wS together:
               refMapMod.mdpStruc.indexP.(fields4MDPs{9}) = optp; 
               optPar(ptCnt, 9)  = optp;
           end  
