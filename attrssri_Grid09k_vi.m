function optPar = attrssri_Grid09k_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest)
%  optPar = attrssribGrid09k_vi( block, wave) :  MAP fitting, with weakly informative priors given by 
% attrssriaGrid09k_vi_**  MAP fitting, with weakly informative priors given by 
%                        scaled beta distributions. ssria means the first wave, and 09b means
%                        that it's the 09 version of the generative function, k for a large set
...
try blocksize; catch blocksize = 18;  end
try 
    blocksize; 
catch 
    if strcmp(wave,'a')
...
for ptN = todo
    ptCnt = ptCnt+1; 
    parOrdGrid = [1 2 9 8 3 5 11 4 6 7 9 10 1 2 8 3 5 7 6 4 9 1 2 11];  % in order above in hd. Cycle twice!
