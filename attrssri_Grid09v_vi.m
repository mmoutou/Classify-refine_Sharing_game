function optPar = attrssri_Grid09v_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest )
%  optPar = attrssribGrid09v_vi( block, wave) :  MAP fitting, with weakly informative priors given by 
% attrssriaGrid09v_vi_**  MAP fitting, with weakly informative priors given by 
%                        scaled beta distributions. ssria means the first wave, and 09v means
%                        that it's the 09 version of the generative function, v for 
%                        using only attribution measures, and set
%                        of parameters to be fitted/considered EXCLUDING POC BIAS, and
%                        setting (transf) wS, i.e. wSTrRef = 0.34004; 
...
fitStr = '09v'; % short string to identify the fitting version in outputs / filenames
...
datNPerPt = 2 * 4 * 12;  % {data = HI, SI ratings} x 4 partners x 12 trials per partner
load('ptIDsAB.mat');
...
par2fitN=(indexPLen-1)+1;  % num of free params at pt level. Don't fit wS, but yes to all the rest, plus othLR.
                % We would like to have the choice to fit any of the params in modStuc.indexP
                % but not nessarily all of them. e.g. length(fields(modStruc.indexP)) = 10
                % but only fit 1st 6 plus the leaning rate from partner to partner, othLR
...
for ptN = todo
    ptCnt = ptCnt+1; 
    % In 09v, no pocB (12) nor and fixed wS (9) : 
    parOrdGrid = [1 2 8 3 11 4 6 7 5 10 1 2 8 3 7 6 4 5 10 1 2 11];  % in order above in hd. Cycle twice!
    ...
    for par2grid = parOrdGrid
        ...
        if parCnt == 1
           ...
           if isempty(ldat)    % if no good init. cond. loaded
                ...
                mapMod.mdpStruc.indexP.wS = wSTrRef;
                % mapMod.mdpStruc.indexP.initEvRat = initEvRatTrRef;   % fixed value 'removing the param'
           end
        ...
        for iGr = -grN:grN      
            ...
            %%  ################ Fixed values ###############
            pocBTr = pocBTrRef;    mapMod.mdpStruc.pocBTr = pocBTrRef;  % fixed unbiased value on real line
            mapMod.mdpStruc.indexP.wS = wSTrRef;                     % wS fixed to reference valud from previous fit
            % mapMod.mdpStruc.indexP.initEvRat = initEvRatTrRef;         
        ...
        refMapMod.mdpStruc.pocBTr = mapMod.mdpStruc.pocBTr;  % pocB is constant
        ...
        %  AIC_kc derived very slopily indeed, but c=0 here, as no group-level-derived pars in 09u:
