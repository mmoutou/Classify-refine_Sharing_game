function optPar = attrssri_Grid09s_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest)
%  optPar = attrssribGrid09s_vi( block, wave) :  MAP fitting, with weakly informative priors given by 
% attrssriaGrid09s_vi_**  MAP fitting, with weakly informative priors given by 
%                        scaled beta distributions. 09s means
%                        that it's the 09 version of the generative function, s for a set
%                        of parameters to be fitted/considered excluding pocBias, setting 
%                        (transf) wS to the median of prev. best fit wS_med_k = 0.34004, and
%                        fixing initEvRat to 1.
%                        and changing BIC to be (derived fairly rigorously from iBIC approx):
%                 BIC_kc := -2 ln Lopt + (k-c) ln(n/2*pi) + (c/N) ln (nN)
%                         = -2 ln Lopt + (k-c)*(ln n - ln(2*pi)) + (c/N) ln (nN)
%  AIC_kc derived very slopily indeed:
...
% 1. parNOTTOGRID 2. par2fitN  3. parOrdGrid 4. fitFileName 5. If need be, if par2grid <= par2fitN-1... statementS
...
par2fitN=(indexPLen-2)+1;  % num of free params at pt level. Don't fit wS, initEvRat (hence -2)
                % but yes to all the rest, plus othLR (but not pocB), so +1.
...
for ptN = todo
    ptCnt = ptCnt+1; 
    % In 09s, no initEvRat (which is 5), no wS (9), no pocB (12) : 
    parOrdGrid = [1 2 8 3 11 4 6 7 10 1 2 8 3 7 6 4 10 1 2 11];  % in order above in hd. Cycle twice!
    ...
    for par2grid = parOrdGrid
        ...
        if parCnt == 1
           ...
           if isempty(ldat)    % if no good init. cond. loaded
                ...
                mapMod.mdpStruc.indexP.wS = wSTrRef;   % fixed value derived from whole group prev. fit
...
%                 end
        ...
        %  AIC_kc derived very slopily indeed. c=1 as only wS is a fixed value param, to group median:
        %         AIC_kc  := -2 ln Lopt + 2*((k-c)  + c/N)
...
tStart
tEnd
save([fitFileName '_done.mat'],'tStart','tEnd','parOrdGrid','grSc');
