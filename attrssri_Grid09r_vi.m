function optPar = attrssri_Grid09r_vi( block, wave, startFitFile, grN, grSc, blocksize, codeTest )
%  optPar = attrssribGrid09r_vi( block, wave, startFitFile) :  MAP fitting, with weakly informative priors 
% attrssriaGrid09r_vi_**  MAP fitting, with weakly informative priors given by 
%                        scaled beta distributions. ssria means the first wave, and 09r means
%                        that it's the 09 version of the generative function, o for a large set
%                        of parameters to be fitted/considered EXCLUDING POC BIAS 
%                        and setting (transf) dInitEv to the median of prev. best fit dInitEv_med_k = 3.080
...
fitStr = '09r';   % short string to identify the fitting version in outputs / file
...
for ptN = todo
    ptCnt = ptCnt+1; 
    % In 09r, no pocB (12) nor dInitEv, which is 3 : 
    parOrdGrid = [1 2 8 5 11 4 6 7 9 10 1 2 8 5 7 6 4 9 10 1 2 11];  % in order above in hd. Cycle twice!
    ...
    for par2grid = parOrdGrid
        ...
        if parCnt == 1
           ...
           if isempty(ldat)    % if no good init. cond. loaded
                ...
                mapMod.mdpStruc.indexP.dInitEv = dInitEvTrRef;   % fixed value derived from whole group prev. fit
        ...
        for iGr = -grN:grN      
            ...
            mapMod.mdpStruc.indexP.dInitEv = dInitEvTrRef;  % fixed value from previous fit
